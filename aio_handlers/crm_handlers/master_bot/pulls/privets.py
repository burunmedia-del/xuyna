...
@router.message(lambda x: 'Приветка' in x.html_text)
async def privet_message_handler(message: Message, state: FSMContext):
    pull_id = int(message.text.split()[1])
    privet_id = int(message.text.split()[2])
    privet = db.get_privet_by_id_and_pull(privet_id, pull_id)
    await message.answer(privet['text'], reply_markup=build_privet_keyboard(privet['keyboard'],'t.me', True, privet_id, pull_id))

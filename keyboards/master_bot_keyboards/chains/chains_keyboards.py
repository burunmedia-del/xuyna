...
def del_chain_key(chain_id, name):
    keyboard = InlineKeyboardBuilder()
    keyboard.row(InlineKeyboardButton(text=f'Удалить цепь «{name}»', callback_data=f'DLCHN_{chain_id}'), width=1)
    keyboard.row(InlineKeyboardButton(text='Вернуться в главное меню', callback_data='MENU'), width=1)
    return keyboard.as_markup()

def chain_remote_key(chain_id, chain_start=False):
    keyboard = InlineKeyboardBuilder()
    ...
    keyboard.row(InlineKeyboardButton(text='Приветка на посл. звене', callback_data=f'privetoflastelement_{chain_id}'),
                InlineKeyboardButton(text='👁', callback_data=f'viewlastprivet_{chain_id}'), width=2)
    ...
    if not chain_start:
        ...
        keyboard.row(InlineKeyboardButton(text='Цепь работает ✅', callback_data='x'), width=1)
